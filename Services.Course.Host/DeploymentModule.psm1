...
#endregion
#region SplunkLogging
function Deployment-GetLog4NetConfiguration
{
                param(
                                [string]$splunkHost,
                                [string]$splunkPort,
                                [string]$threshold = "INFO",
                                [string]$identity = "OctopusDeploy"
                )
                $configString = @"
<log4net>
  <appender name="SplunkAppender" type="log4net.Appender.RemoteSyslogAppender">
    <remoteAddress value="$splunkHost" />
...
"@
                $configXml = [xml]$configString
                return $configXml
}
function Deployment-LogInfoToSplunk
{
                param(
                                [string]$message,
                                [string]$identity = "OctopusDeploy"
                )
                $dlls = Get-ChildItem $PSScriptRoot -Include "log4net.dll" -Recurse
                if ($dlls -ne $null -and $dlls.Length -gt 0)
                {
                	$splunkUrlVariable = Get-Variable -Name "SplunkUrl" -ErrorAction SilentlyContinue
                	$splunkUrlPortVariable = Get-Variable -Name "SplunkUrlPort" -ErrorAction SilentlyContinue
                	if (-not $splunkUrlVariable -or -not $splunkUrlPortVariable)
                	{
                	}
                	else
                	{
                                if ($SplunkUrl -ne $null -and $SplunkUrlPort -ne $null)
                                {
                                                # load the log4net assembly
                                                [void][Reflection.Assembly]::LoadFile($dlls[0].FullName)
                                                # get the configuration
                                                $log4netConfig = Deployment-GetLog4NetConfiguration -SplunkHost $SplunkUrl -SplunkPort $SplunkUrlPort -Threshold "INFO" -Identity $identity
                                                [log4net.LogManager]::ResetConfiguration()
                                                [log4net.Config.XmlConfigurator]::Configure($log4netConfig.get_DocumentElement())
                                                # get the logger
...
function Deployment-LogErrorToSplunk
{
                param(
                                [string]$message,
                                [string]$identity = "OctopusDeploy"
                ...
                if ($dlls -ne $null -and $dlls.Length -gt 0)
                {
                	...
                	else
                	{
                                if ($SplunkUrl -ne $null -and $SplunkUrlPort -ne $null)
                                {
                                                ...
                                                [void][Reflection.Assembly]::LoadFile($dlls[0].FullName)
                                                # get the configuration
                                                $log4netConfig = Deployment-GetLog4NetConfiguration -SplunkHost $SplunkUrl -SplunkPort $SplunkUrlPort -Threshold "ERROR" -Identity $identity
                                                [log4net.LogManager]::ResetConfiguration()
                                                [log4net.Config.XmlConfigurator]::Configure($log4netConfig.get_DocumentElement())
                                                # get the logger
                                                $log = [log4net.LogManager]::GetLogger($Host.GetType())
